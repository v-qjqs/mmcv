...
class TestPhotometric:
    ...
    def test_adjust_contrast(self, nb_rand_test=100):
        ...
        def _adjust_contrast(img, factor):
            ...
            # cons_obj = Contrast(img)
            # print("cons_obj.degen: ", np.asarray(cons_obj.degenerate))
            # mean = int(ImageStat.Stat(image.convert("L")).mean[0] + 0.5)

            # print("PIL gray_img: ", np.asarray(img.convert("L")))
            # print('------------------------')
            # b, g, r = contrasted_img.split()
            # contrasted_img = Image.merge("BGR", (r, g, b))
