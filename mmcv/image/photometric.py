...
def adjust_brightness(img, alpha=1.):
    ...
    brightened_img = cv2.addWeighted(
        ...
        1 - alpha, 0)
    # brightened_img = cv2.addWeighted(img, alpha, degenerated, 1-alpha, 0)
...
def adjust_contrast(img, alpha=1.):
    ...
    Args:
        ...
        alpha (float): Same as :func:`mmcv.adjust_brightness`.
    ...
    # mean = int(np.sum(gray_img) / np.sum(hist))
    # mean = round(sum((j * hist[j] for j in range(256)))/np.sum(hist))
    # print("mean: ", mean, round(np.sum(gray_img) / np.sum(hist)))
    # mean = round(np.sum(gray_img) / np.sum(hist))
    # mean = np.sum(gray_img) / np.sum(hist)
    degenerated = (np.ones_like(img[..., 0]) * mean).astype(img.dtype)
    # degenerated = gray2bgr(degenerated).astype(np.float32)
    # degenerated = np.clip(degenerated, 0, 255)
    degenerated = gray2bgr(degenerated)  # .astype(np.float32)
    # TODO check need clip?
    # mean = np.clip(mean, 0, 255)
    # degenerated = np.clip(degenerated, 0, 255)
    # degenerated = np.ones_like(img) * mean
